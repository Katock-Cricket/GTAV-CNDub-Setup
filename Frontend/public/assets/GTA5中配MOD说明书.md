...
1. **三主角和自机还有一些语音没有中配**：顾不上弄了，鸽了半年也没憋出来，主要演员们都没时间，以后随缘更新了；
2. **“掌上明珠”以后的主线没有中配**：原因同上；
3. **使用安装器对增强版安装后不生效或无法启动**：如果你之前没有安装过任何MOD，游戏版本高于上面通过测试的版本，则可能是游戏更新导致的，与本MOD无关，请手动更新`OpenRPF.asi`、`xinput1_4.dll`等；如果你本身就有asi loader，请检查是不是安装重了，asi loader一般命名为`xinput1_4.dll`、`dsound.dll`等，只保留一个可以用的就行；
4. **安装器无法启动**：安装器有两项依赖，见上文。如果你电脑比较老，可能没有，启动安装器时会弹窗提示，请先到`pre_install`中运行对应的安装程序；
5. **BattlEye相关报错**：安装任何模组后，GTA5可能因BattlEye(反作弊程序)检测到额外加载项而无法正常启动，请尝试在R星启动器中禁用BE反作弊，或在exe的“启动选项”的里添加-nobattleye参数。禁用后无法使用线上功能，也就是说模组只能线下玩，不能与OL共存；
6. **ScriptHookV相关报错**：中配MOD不依赖于ScriptHookV，任何与ScriptHookV有关的报错均与中配MOD无关；
7. **增强版主线任务字幕不匹配**：由于增强版文件格式是Gen9，目前只有CodeWalker+OpenRPF提供了转换+挂载的功能，但还不完整，导致字幕依然不能顺利加载，我能力范围内做不到这点，所以增强版不能安装配套字幕，安装器也会自动跳过这项。
